...
3.  **Annotated Tag**:
    ```powershell
    git tag -a SRAM-v<VERSION> -m "Release SRAM-v<VERSION>"
    ```
...
1.  **Switch & Merge**:
    ```powershell
    git checkout main
    git merge SRAM-v<VERSION> --ff-only
    ```
2.  **Push**:
    ```powershell
    git push origin main SRAM-v<VERSION>
    ```
3.  **Return**:
    ```powershell
    git checkout -
...
1.  **GitHub Release**: Create a release targeting tag `v<VERSION>`. Upload both zips.
    ```powershell
    # NOTE: Run from root repo if SRAM is nested
    gh release create SRAM-v<VERSION> StandaloneReactionAnimationMod_PCVR_v<VERSION>.zip StandaloneReactionAnimationMod_Nomad_v<VERSION>.zip --title "SRAM-v<VERSION>" --notes "See manifest.json for details."
    ```
