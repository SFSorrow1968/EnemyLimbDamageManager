# EnemyLimbDamageManager Publishing Script
# Usage: ./publish.ps1
...
$projectPath = "EnemyLimbDamageManager.csproj"
...
if (-not (Test-Path "bin/PCVR/EnemyLimbDamageManager/EnemyLimbDamageManager.dll")) { Write-Error "PCVR Build Failed: DLL not found." }
if (-not (Test-Path "bin/Nomad/EnemyLimbDamageManager/EnemyLimbDamageManager.dll")) { Write-Error "Nomad Build Failed: DLL not found." }

# 4. Package
Write-Host "4. Packaging..." -ForegroundColor Cyan
$pcvrZip = "EnemyLimbDamageManager_PCVR_v$version.zip"
$nomadZip = "EnemyLimbDamageManager_Nomad_v$version.zip"

if (Test-Path $pcvrZip) { Remove-Item $pcvrZip }
if (Test-Path $nomadZip) { Remove-Item $nomadZip }

Compress-Archive -Path "bin/PCVR/EnemyLimbDamageManager/*" -DestinationPath $pcvrZip
Compress-Archive -Path "bin/Nomad/EnemyLimbDamageManager/*" -DestinationPath $nomadZip
